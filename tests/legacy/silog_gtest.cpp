...
class MpscQueueTest : public ::testing::Test {
  protected:
    void SetUp() override { ASSERT_EQ(SILOG_OK, SilogMpscQueueInit(&queue, sizeof(TestItem), 1024)); }
...
{
    TestItem item = {42};
    ASSERT_EQ(SILOG_OK, SilogMpscQueuePush(&queue, &item));

    TestItem out;
    ASSERT_EQ(SILOG_OK, SilogMpscQueuePop(&queue, &out));
...
{
    TestItem out;
    EXPECT_NE(SILOG_OK, SilogMpscQueuePop(&queue, &out));
}
...
TEST_F(MpscQueueTest, MultiThread)
{
    ...
    for (int i = 0; i < producerCount; i++) {
        producers.emplace_back([this, i, perThread, &totalPushed]() {
            for (int j = 0; j < perThread; j++) {
                ...
                if (SilogMpscQueuePush(&queue, &item) == SILOG_OK) {
                    totalPushed++;
                }
            }
        });
    ...
    while (consecutiveEmpty < 100) { /* 连续 100 次空则认为结束 */
        if (SilogMpscQueuePop(&queue, &out) == SILOG_OK) {
            popCount++;
            consecutiveEmpty = 0;
        } else {
            consecutiveEmpty++;
